          ...
          <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as 'form' | 'custom')}>
            ...
            <TabsContent value="form" className="mt-6" forceMount hidden={activeTab !== 'form'}>
              <FormMCPConfig
                serverId={serverId}
                onClose={onClose}
                onConfigChange={activeTab === 'form' ? setPreviewConfig : undefined}
              />
            </TabsContent>
            ...
            <TabsContent value="custom" className="mt-6" forceMount hidden={activeTab !== 'custom'}>
              <CustomMCPConfig
                serverId={serverId}
                onClose={onClose}
                onConfigChange={activeTab === 'custom' ? setPreviewConfig : undefined}
              />
            </TabsContent>
          </Tabs>
        </div>
          ...
          {previewConfig ? (
            <Card className="border border-border bg-muted/30">
              <CardContent className="pt-6 space-y-3">
                ...
                {previewConfig.headers && Object.keys(previewConfig.headers).length > 0 && (
                  <div>
                    <p className="text-xs text-muted-foreground mb-1">
                      {t('config.form.headers_preview')}
                    </p>
                    <div className="text-sm font-mono space-y-1">
                      {Object.entries(previewConfig.headers).map(([key, value]) => (
                        <p key={key} className="text-xs">
